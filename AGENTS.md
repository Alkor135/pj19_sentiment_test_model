...
- `trade/` — копия QUIK trade-скриптов из `pj18_sentiment/trade`. Скрипты используют `trade/settings.yaml` для QUIK-аккаунтов и объёмов, а параметры тикеров/прогнозов берут из `<ticker>/settings.yaml` через `<ticker>/config_loader.py`.
...
- `<ticker>/run_<ticker>.py` — обнаруживает `<model>/run_report.py` через `iterdir()` и запускает их по очереди. Поддерживает `--only` (фильтр моделей) и `--keep-going` (не останавливаться при падении одной модели).
...
- `trade/` — локальные скрипты выставления заявок через QUIK `.tri` и Lua-экспорт текущих минуток/позиций. Lua-файлы пишут в `trade/quik_export/`; runtime-файлы `trade/log/`, `trade/state/`, `trade/quik_export/*.csv/json/tmp` игнорируются git.

**Конфиг тикера:** `<ticker>/settings.yaml` — секционный файл с полной картиной настроек тикера. `common` содержит тикерные параметры, внешние пути и единые окна дат `stats_date_from`/`stats_date_to`/`backtest_date_from`/`backtest_date_to`; `shared` — параметры подготовки данных; `model_defaults` — общие настройки моделей; `models` — различия конкретных моделей; `combine` — параметры объединённого сигнала. Скрипты читают его через `<ticker>/config_loader.py`, который применяет подстановки `{ticker}`, `{ticker_lc}`, `{model_dir}`.

## Соглашения, которые не очевидны из кода

**Никаких хардкоженных имён тикеров в скриптах.** Скрипты определяют свой тикер/модель через расположение файла и `<ticker>/config_loader.py`. При копировании в новую модель правится только секция `models` в `<ticker>/settings.yaml`.
