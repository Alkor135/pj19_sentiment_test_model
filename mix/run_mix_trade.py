"""
Торговый оркестратор всех модельных пайплайнов для тикера MIX.
...
После успешной подготовки скрипт находит в подпапках `mix/<model>/` файлы `run_trade.py`
...
запускает combine-пайплайн в `mix/combine/run_trade.py` (если папка есть):
...
Запуск:
python mix/run_mix_trade.py
python mix/run_mix_trade.py --only gemma3_12b,gemma4_e2b --keep-going
python mix/run_mix_trade.py --keep-going
"""
