"""
Отчётный оркестратор всех модельных пайплайнов для тикера MIX.
...
запускает combine-пайплайн в `mix/combine/run_report.py` (если папка есть):
...
его выполняет торговый оркестратор run_mix_trade.py на основной машине.
...
python mix/run_mix_report.py --only gemma3_12b,gemma4_e2b,gemma4_e4b,qwen2.5_14b,qwen2.5_7b,combine --keep-going
"""
