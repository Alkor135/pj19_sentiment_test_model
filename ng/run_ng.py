"""
Оркестратор всех модельных пайплайнов для тикера NG.

Скрипт находит в подпапках `ng/<model>/` файлы `run_report.py`
...
запускает combine-пайплайн в `ng/combine/` (если эта папка есть):
...
Запуск:
python ng/run_ng.py
python ng/run_ng.py --only gemma3_12b,gemma4_e2b,gemma4_e4b,qwen2.5_14b,qwen2.5_7b,qwen3_14b --keep-going
python ng/run_ng.py --keep-going  «продолжать дальше»
"""
...
app = typer.Typer(help="Последовательный запуск пайплайнов всех моделей NG.")


def discover_model_runners() -> list[Path]:
    """Возвращает отсортированный список оркестраторов вида ng/<model>/run_report.py."""
    runners: list[Path] = []
...
def main(
    ...
    """Прогоняет пайплайны всех моделей NG или подмножества по --only."""
