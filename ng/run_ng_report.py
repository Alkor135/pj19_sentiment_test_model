"""
Отчётный оркестратор всех модельных пайплайнов для тикера NG.
...
запускает combine-пайплайн в `ng/combine/run_report.py` (если папка есть):
...
его выполняет торговый оркестратор run_ng_trade.py на основной машине.
При запуске с `--only` combine выполняется только если явно указан в списке.

Запуск:
python ng/run_ng_report.py
