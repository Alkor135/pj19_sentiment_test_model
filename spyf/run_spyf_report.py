"""
Отчётный оркестратор всех модельных пайплайнов для тикера SPYF.
...
запускает combine-пайплайн в `spyf/combine/run_report.py` (если папка есть):
...
его выполняет торговый оркестратор run_spyf_trade.py на основной машине.
