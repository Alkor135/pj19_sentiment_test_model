"""Генератор Excel и HTML отчётов по walk-forward результатам.

Модуль читает `summary.csv` и `trades.csv` из `walk_forward/results/`, строит
leaderboard, сводки по тикерам, месячные/дневные матрицы, Excel workbook и
самодостаточный HTML dashboard с графиками Plotly.
...
Примеры запуска:
    ...
    .venv\\Scripts\\python.exe -m walk_forward.report --chrome-path "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe"
"""
...
def normalize_summary(summary: pd.DataFrame) -> pd.DataFrame:
    """Приводит summary DataFrame к ожидаемым колонкам и типам."""
...
def normalize_trades(trades: pd.DataFrame) -> pd.DataFrame:
    """Приводит trades DataFrame к ожидаемым колонкам и типам."""
...
def _empty_leaderboard() -> pd.DataFrame:
    """Возвращает пустой leaderboard с фиксированным набором колонок."""
...
def _metric_row(key: tuple[Any, ...], summary: pd.DataFrame, trades: pd.DataFrame) -> dict[str, Any]:
    """Считает строку метрик для одной пары тикер/модель."""
...
def build_leaderboard(summary: pd.DataFrame, trades: pd.DataFrame) -> pd.DataFrame:
    """Строит рейтинг моделей внутри каждого тикера по score и total_pnl."""
...
def build_ticker_summary(leaderboard: pd.DataFrame) -> pd.DataFrame:
    """Агрегирует leaderboard в компактную сводку по тикерам."""
...
def _empty_errors() -> pd.DataFrame:
    """Возвращает пустую таблицу ошибок с ожидаемыми колонками."""
...
def _resolve_ticker_dir(results_dir: Path, ticker: str) -> Path:
    """Находит папку тикера в results_dir без чувствительности к регистру."""
...
def load_all_trades(results_dir: Path, summary: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Читает все `trades.csv` по ok-строкам summary и собирает ошибки чтения."""
...
def _build_period_matrix(trades: pd.DataFrame, period_column: str) -> pd.DataFrame:
    """Строит pivot-матрицу P/L по модели и периоду."""
...
def build_monthly_matrix(trades: pd.DataFrame) -> pd.DataFrame:
    """Строит месячную матрицу P/L по тикерам и моделям."""

    return _build_period_matrix(trades, "month")


def build_daily_matrix(trades: pd.DataFrame) -> pd.DataFrame:
    """Строит дневную матрицу P/L по тикерам и моделям."""
...
def build_dashboard(
    ...
    """Готовит компактный набор KPI для листа Dashboard в Excel."""
...
def _excel_safe(frame: pd.DataFrame) -> pd.DataFrame:
    """Возвращает копию таблицы перед записью в Excel."""

    return frame.copy()


def _sheet_name(value: str, used: set[str]) -> str:
    """Создаёт безопасное и уникальное имя листа Excel."""
...
def write_excel_report(
    *,
    ...
    """Записывает полный Excel workbook со сводками, матрицами и raw-данными."""
...
def _style_workbook(path: Path) -> None:
    """Применяет базовое форматирование к созданному Excel workbook."""
...
def _table_html(frame: pd.DataFrame, max_rows: int = 50, index: bool = False) -> str:
    """Преобразует DataFrame в HTML-таблицу с ограничением числа строк."""
...
def _kpi_html(label: str, value: Any) -> str:
    """Рендерит один KPI-блок для HTML dashboard."""

    return f'<div class="kpi"><span>{escape(label)}</span><strong>{escape(str(value))}</strong></div>'


def _format_float(value: Any) -> str:
    """Форматирует числовое значение для человекочитаемого HTML."""
...
def _plot_div(fig: go.Figure, *, include_plotlyjs: bool) -> str:
    """Преобразует Plotly Figure в HTML-фрагмент."""
...
def _overall_daily_pnl(trades: pd.DataFrame) -> pd.Series:
    """Считает общий дневной P/L по всем сделкам."""
...
def _equity_figure(trades: pd.DataFrame) -> go.Figure:
    """Создаёт график суммарной equity curve."""
...
def _drawdown_figure(trades: pd.DataFrame) -> go.Figure:
    """Создаёт график просадки по суммарной equity curve."""
...
def _daily_bar_figure(trades: pd.DataFrame) -> go.Figure:
    """Создаёт bar chart дневного P/L."""
...
def _monthly_heatmap_figure(monthly_matrix: pd.DataFrame) -> go.Figure:
    """Создаёт heatmap месячной матрицы P/L."""
...
def _ticker_equity_figure(ticker: str, trades: pd.DataFrame, leaderboard: pd.DataFrame) -> go.Figure:
    """Создаёт equity-график top-5 моделей выбранного тикера."""
...
def build_html(
    *,
    ...
    """Собирает полный HTML dashboard с KPI, графиками и таблицами."""
...
def _summary_error_rows(summary: pd.DataFrame) -> pd.DataFrame:
    """Преобразует skipped/error строки summary в единую таблицу ошибок."""
...
def build_report(
    *,
    ...
    """Строит Excel и HTML отчёты из папки walk-forward результатов."""
...
def open_html_in_chrome(chrome_path: Path, output_html: Path) -> None:
    """Открывает HTML отчёт в новом окне Google Chrome."""
...
def main(
    ...
    """Точка входа CLI: собирает отчёты и при необходимости открывает HTML."""
