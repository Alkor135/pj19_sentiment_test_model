"""CLI-оркестратор дневного walk-forward бэктеста.

Скрипт читает `walk_forward/settings.yaml`, настройки тикеров и готовые
`sentiment_scores.pkl`, затем запускает расчёт по тикерам/моделям без записи
в рабочие модельные папки. Все результаты пишутся в `walk_forward/results/`
или в папку, переданную через `--output-dir`.

Примеры запуска:

    .venv\\Scripts\\python.exe -m walk_forward.run_walk_forward
    .venv\\Scripts\\python.exe -m walk_forward.run_walk_forward --tickers rts --models gemma3_12b
    .venv\\Scripts\\python.exe -m walk_forward.run_walk_forward --start-date 2026-04-01 --train-months 6
    .venv\\Scripts\\python.exe -m walk_forward.run_walk_forward --no-save-daily-artifacts --keep-going
"""
...
@dataclass
class RunOptions:
    """Собранные настройки одного запуска walk-forward CLI."""
...
def parse_csv(value: Optional[str], default: tuple[str, ...] = ()) -> list[str]:
    """Разбирает CLI-строку через запятую или возвращает default."""
...
def deep_merge(base: dict[str, Any], override: dict[str, Any]) -> dict[str, Any]:
    """Рекурсивно объединяет словари настроек без изменения исходников."""
...
def apply_template_values(settings: dict[str, Any], model_dir: str) -> dict[str, Any]:
    """Подставляет `{ticker}`, `{ticker_lc}` и `{model_dir}` в строках настроек."""
...
def parse_date(value: Any) -> date:
    """Преобразует YAML/CLI значение даты в `datetime.date`."""
...
def load_yaml(path: Path) -> dict[str, Any]:
    """Читает YAML-файл и возвращает пустой словарь для пустого файла."""
...
def load_ticker_settings(ticker_lc: str) -> dict[str, Any]:
    """Загружает `<ticker>/settings.yaml` по нижнему имени тикера."""

    return load_yaml(ROOT / ticker_lc / "settings.yaml")


def discover_models(raw_ticker_settings: dict[str, Any]) -> list[str]:
    """Возвращает список моделей, описанных в секции `models` тикера."""
...
def build_model_settings(raw_ticker_settings: dict[str, Any], model_dir: str) -> dict[str, Any]:
    """Собирает итоговые настройки модели из common/defaults/model override."""
...
def merge_run_options(
    ...
    *,
    ...
    """Объединяет настройки YAML и CLI-переопределения в `RunOptions`."""
...
def index_by_date(df: pd.DataFrame) -> pd.DataFrame:
    """Проверяет уникальность дат и делает `source_date` индексом DataFrame."""
...
def load_sentiment_pkl(path: Path) -> pd.DataFrame:
    """Загружает sentiment PKL и возвращает индексированный по датам DataFrame."""
...
def error_summary(ticker: str, model_dir: str, sentiment_model: str, error: Exception) -> dict[str, Any]:
    """Формирует summary-строку об ошибке запуска модели."""
...
def main(
    ...
    """Точка входа CLI: запускает walk-forward по выбранным тикерам и моделям."""
