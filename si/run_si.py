"""
Оркестратор всех модельных пайплайнов для тикера Si.
...
запускает combine-пайплайн в `si/combine/` (если эта папка есть):
...
Запуск:
python si/run_si.py
python si/run_si.py --only gemma3_12b,gemma4_e2b,gemma4_e4b,qwen2.5_14b,qwen2.5_7b,qwen3_14b --keep-going
python si/run_si.py --keep-going  «продолжать дальше»
"""
...
app = typer.Typer(help="Последовательный запуск пайплайнов всех моделей Si.")


def discover_model_runners() -> list[Path]:
    """Возвращает отсортированный список оркестраторов вида si/<model>/run_report.py."""
    runners: list[Path] = []
...
def main(
    ...
    """Прогоняет пайплайны всех моделей Si или подмножества по --only."""
