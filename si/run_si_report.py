"""
Отчётный оркестратор всех модельных пайплайнов для тикера SI.

Скрипт находит в подпапках `si/<model>/` файлы `run_report.py`
...
запускает combine-пайплайн в `si/combine/run_report.py` (если папка есть):
...
его выполняет торговый оркестратор run_si_trade.py на основной машине.
При запуске с `--only` combine выполняется только если явно указан в списке.

Запуск:
python si/run_si_report.py
python si/run_si_report.py --only gemma3_12b,gemma4_e2b --keep-going
python si/run_si_report.py --keep-going
python si/run_si_report.py --only gemma3_12b,gemma4_e2b,gemma4_e4b,qwen2.5_14b,qwen2.5_7b,gpt-oss_20b,combine
python si/run_si_report.py --only gemma3_12b,gemma4_e2b,gemma4_e4b,qwen2.5_14b,qwen2.5_7b,combine --keep-going
"""
