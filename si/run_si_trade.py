"""
Торговый оркестратор всех модельных пайплайнов для тикера SI.
...
После успешной подготовки скрипт находит в подпапках `si/<model>/` файлы `run_trade.py`
...
запускает combine-пайплайн в `si/combine/run_trade.py` (если папка есть):
...
Запуск:
python si/run_si_trade.py
python si/run_si_trade.py --only gemma3_12b,gemma4_e2b --keep-going
python si/run_si_trade.py --keep-going
"""
