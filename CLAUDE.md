...
**Торговый пайплайн внутри модели** — 5 шагов, каждый — отдельный typer-CLI:
...
**Отчётный модельный пайплайн** — первые 4 шага без `sentiment_to_predict.py`. Он входит в `<ticker>/<model>/run_report.py`; генерация прогнозов выполняется только торговым оркестратором `<ticker>/<model>/run_trade.py` на основной торговой машине.

**Combine-пайплайн (`<ticker>/combine/`):** склеивает результаты двух моделей.
- Настройки combine лежат в секции `combine` файла `<ticker>/settings.yaml`: `model_1`, `model_2` (имена папок моделей; допустимы и Ollama-имена с `:`, скрипт нормализует через `replace(":", "_")`), `notional_capital` (для QuantStats), `predict_path`.
...
**Никаких хардкоженных имён тикеров в модельных/shared/combine-скриптах.** Скрипты определяют свой тикер/модель через расположение файла и `<ticker>/config_loader.py`. EBS trade-скрипты являются отдельными счётными адаптерами для `mix`/`rts` и явно задают `ticker_lc`.
...
**При добавлении новой модели для тикера:** скопировать существующую папку-модель и добавить запись в секцию `models` файла `<ticker>/settings.yaml` (`sentiment_model` и при необходимости переопределения). Корневой оркестратор `run_<ticker>.py` подхватит её автоматически (ищет `run_report.py` в подпапках).

**При добавлении нового тикера:** скопировать структуру `rts/` целиком, в новом `<ticker>/settings.yaml` обновить `ticker`/`ticker_lc`/`ticker_close`/`ticker_open`, переименовать `run_<ticker>.py`. Регулярная замена `rts`→`<new>` опасна: она цепляет английские слова `startswith`, `reports` и т.п. Использовать regex с не-буквенными границами: `(?<![a-zA-Z])rts(?![a-zA-Z])`.
